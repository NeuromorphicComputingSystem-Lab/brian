...
class Morphology(object):
    ...
    def plot(self, axes=None, simple=True, origin=None, values=None, max_val=None, min_val=None, offset=0):
        """
		...
		values : an array of scalars. adjust color according to the relative values. ignored if simple is True.
		offset : used to pick the right index in values
		"""
        if values != None:
            maximum = max(values)
            minimum = min(values)
        ...
        if len(x) == 1: # root with a single compartment: probably just the soma
            ...
            if simple :
                if values == None :
                    axes.plot(x, y, z, 'k')
                else : # Remy's code: color plot (imaging)
                    mean_value = 0 
                    for n in range(1, len(x)):
                        mean_value += values[offset+n-1]
                    mean_value /= len(x)
                    if max_val==None:
                        axes.plot(x, y, z, color=((mean_value-minimum)/Range,0,(maximum-mean_value)/Range))
                    else:
                        val = min(mean_value,max_val)
                        val = max(val,min_val)
                        Range = max_val - min_val
                        axes.plot(x, y, z, color=((val-min_val)/Range,0,(max_val-val)/Range))
                ...
                for n in range(1, len(x)):
                    if values==None :
                        ...
                        if max_val==None:
                            axes.plot([x[n - 1], x[n]], [y[n - 1], y[n]], [z[n - 1], z[n]], color=((values[offset+n-1]-minimum)/Range,0,(maximum-values[offset+n-1])/Range), linewidth=d[n - 1])
                        else:
                            val = min(values[offset+n-1],max_val)
                            val = max(val,min_val)
                            Range = max_val - min_val
                            axes.plot([x[n - 1], x[n]], [y[n - 1], y[n]], [z[n - 1], z[n]], color=((val-min_val)/Range,0,(max_val-val)/Range), linewidth=d[n - 1])
                        #axes.plot(x, y, z, color=((values[offset+n-1]-minimum)/Range,0,(maximum-values[offset+n-1])/Range))
        for c in self.children:
            c.plot(origin=(x[-1], y[-1], z[-1]), axes=axes, simple=simple,values=values, max_val=max_val, min_val=min_val,offset = offset + len(x)-1)
...
if __name__ == '__main__':
	...
	morpho=Morphology('D:\Python27\Lib\site-packages\mp_ma_40984_gc2.CNG.swc') # retinal ganglion cell
